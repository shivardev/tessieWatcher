  ...
  const queries = useMemo(() => {
    ...
    return [
      `WITH c AS (SELECT * FROM charging_sessions WHERE status='closed' AND ${filtered}), d AS (SELECT d.*,v.efficiency_wh_km FROM drives d JOIN vehicles v ON v.id=d.vehicle_id WHERE d.status='closed' AND ${timeRangeSql(settings.timeRange,'d.start_time')}),
       ...
       gross AS (SELECT SUM(loss)*(SELECT efficiency_wh_km FROM vehicles LIMIT 1) energy,(SELECT SUM(distance_km) FROM d) distance FROM losses)
       ...
       COALESCE(SUM(cost),0)/NULLIF(SUM(MAX(COALESCE(charge_energy_added_kwh,0),COALESCE(charge_energy_used_kwh,0))),0)*(SELECT (energy/1000.0)/NULLIF(CASE WHEN '${settings.lengthUnit}'='mi' THEN distance/1.60934 ELSE distance END,0)*100 FROM gross),
