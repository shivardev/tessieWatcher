...
Done. Both binaries are fully static (no glibc/cgo dependency) and can
just be copied to the target machine and run directly:
  ...
  Deploying to the Pi (linux/arm64):
...
either binary can be run directly in a terminal against a config.toml
in the current directory. deploy/install.sh (systemd) and Docker are
both optional, for when you're ready to leave it running unattended.
EOF
